...
import sys
sys.path.insert(0, r'D:\Shenime\Shenimet\Data_Science\Master_thesis\github_2\rag-server\src\project')
...
# Load initial documents index and metadata
INITIAL_VECTORSTORE_PATH = "vector_index_fau.faiss"
INITIAL_METADATA_PATH = "knowledgebase/quality_html-pdf.jsonl"

initial_index, initial_metadata = load_vector_db(index_file=INITIAL_VECTORSTORE_PATH, metadata_file=INITIAL_METADATA_PATH)

# (separate from initial_index)
cache_index = faiss.IndexFlatL2(128)  # Ensure the dimension matches your embeddings
cache = SmartCache(index=cache_index)  # Pass the separate cache_index
...
async def create_completion(request: CompletionRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        retrieved_docs = RAG_Retrieval.dense_retrieval(request.prompt, initial_index, initial_metadata, top_k=20)
        ...
        #print(response)
        #cache.append_to_cache(request.prompt, response)
        return response
